...
$exe = Join-Path $output "SignatureReportQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "SignatureReportQa.cs")
& $csc /nologo /target:exe /optimize /main:SignatureReportQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de firmas."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de lectura de firmas ha fallado."
}
