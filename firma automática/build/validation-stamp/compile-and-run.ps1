...
$exe = Join-Path $output "StampQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "StampQa.cs")
& $csc /nologo /target:exe /optimize /main:StampQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de marcado."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de marca de agua y numeracion ha fallado."
}
