# Firmar con la clave privada tal como la entrega Windows (CSP o CNG).
...
$exe = Join-Path $output "CngSigningQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "CngSigningQa.cs")
& $csc /nologo /target:exe /optimize /main:CngSigningQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de firma con clave moderna."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de firma con clave moderna ha fallado."
}
