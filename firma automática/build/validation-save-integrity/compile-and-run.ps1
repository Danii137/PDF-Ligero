...
$exe = Join-Path $output "SaveIntegrityQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "SaveIntegrityQa.cs")
& $csc /nologo /target:exe /optimize /main:SaveIntegrityQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de guardado."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de guardado ha fallado."
}
