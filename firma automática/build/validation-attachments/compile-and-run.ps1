...
$exe = Join-Path $output "AttachmentsQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "AttachmentsQa.cs")
& $csc /nologo /target:exe /optimize /main:AttachmentsQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de adjuntos."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de adjuntos ha fallado."
}
