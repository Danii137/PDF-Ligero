# Zoom con la rueda: respuesta por muesca, fotogramas y anclaje al soltar.
...
$exe = Join-Path $output "SmoothZoomQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "SmoothZoomQa.cs")
& $csc /nologo /target:exe /optimize /main:SmoothZoomQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de zoom suave."
}
...
& $exe $PdfReal
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de zoom suave ha fallado."
}
