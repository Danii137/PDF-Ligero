...
$exe = Join-Path $output "PdfPlanComparisonSurfaceQa.exe"

& $csc `
    /target:exe `
    /optimize `
    /main:FirmaAutomatica.PdfPlanComparisonSurfaceQa `
    "/out:$exe" `
    $referenceArgs `
    $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba UI de comparación."
}
...
& $exe $run
if ($LASTEXITCODE -ne 0) {
    throw "La prueba UI de comparación ha fallado."
}
