...
$exe = Join-Path $output "ShrinkQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "ShrinkQa.cs")
& $csc /nologo /target:exe /optimize /main:ShrinkQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de reduccion."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de reduccion de tamaño ha fallado."
}
