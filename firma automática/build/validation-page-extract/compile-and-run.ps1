...
$exe = Join-Path $output "PageExtractQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "PageExtractQa.cs")
& $csc /nologo /target:exe /optimize /main:PageExtractQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de extraccion."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de extraccion de paginas ha fallado."
}
