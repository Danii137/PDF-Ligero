...
$exe = Join-Path $output "ExportQa.exe"
$appSources = Get-ChildItem -LiteralPath $root -Filter *.cs |
    Select-Object -ExpandProperty FullName
$sources = $appSources + @(Join-Path $validationDir "ExportQa.cs")
& $csc /nologo /target:exe /optimize /main:ExportQa.Program `
    "/out:$exe" $referenceArgs $sources
if ($LASTEXITCODE -ne 0) {
    throw "No se pudo compilar la prueba de exportacion."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "La prueba de exportacion ha fallado."
}
